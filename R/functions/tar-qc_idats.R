...
read_idats <- function(
  ...
  if (filter_beads) {
    ...
    n_beads_discarded <- format(dim(mset)[1] - dim(mset_f2)[1], big.mark = ",", digits = 1L, nsmall = 0L)
    mset <- mset_f2
  }

  if (filter_non_cpg) {
    mset_f2 <- minfi::dropMethylationLoci(mset, dropCH = TRUE)
    n_non_cpg_discarded <- format(dim(mset)[1] - dim(mset_f2)[1], big.mark = ",", digits = 1L, nsmall = 0L)
    mset <- mset_f2
  }
  ...
  if (filter_snps) {
    ...
    n_snps_discarded <- format(dim(mset)[1] - dim(mset_f2)[1], big.mark = ",", digits = 1L, nsmall = 0L)
    mset <- mset_f2
  }
  ...
  if (filter_multihit) {
    ...
    n_multihit_discarded <- format(dim(mset)[1] - dim(mset_f2)[1], big.mark = ",", digits = 1L, nsmall = 0L)
    mset <- mset_f2
  }
  ...
  if (filter_xy) {
    ...
    n_xy_discarded <- format(dim(mset)[1] - dim(mset_f2)[1], big.mark = ",", digits = 1L, nsmall = 0L)
    mset <- mset_f2
  }
  ...
  if (filter_callrate) {
    log_msg <- c(log_msg, paste0(
      ...
      "  - ", format(length(bad_samples), big.mark = ",", digits = 1L, nsmall = 0L), " samples were discarded."
    ))
    log_msg <- c(log_msg, paste0(
      "Filtering probes with call rate below ",
      paste(format(callrate_probes * 100, digits = 1, nsmall = 1), "%"), ":\n",
      "  - ", format(length(bad_cpgs), big.mark = ",", digits = 1L, nsmall = 0L), " probes were discarded."
    ))
  }
  if (filter_beads) {
    log_msg <- c(log_msg, paste0(
  ...
  log_msg <- c(log_msg,
    ...
    paste0(
      "Data contains:\n",
      "  - ", format(dim(methylation_matrix)[1], big.mark = ",", digits = 1L, nsmall = 0L), " probes.\n",
      "  - ", format(dim(methylation_matrix)[2], big.mark = ",", digits = 1L, nsmall = 0L), " samples.\n",
      "  - ", format(sum(is.na(methylation_matrix)), big.mark = ",", digits = 1L, nsmall = 0L), " missing values."
    )
  )
  if (echo) {
