...
epic_annot_pkg_urls <- paste(sprintf("[%s](%s)", epic_annot_pkg_urls, epic_annot_pkg_urls), collapse = ",")
```
...
* R packages:
    * ` `r epic_annot_pkg` ` ` `r as.character(packageVersion(epic_annot_pkg))` ` (`r epic_annot_pkg_urls`)
    * `limma` ` `r cat(as.character(packageVersion("limma")))` ` ([https://bioconductor.org/packages/limma/](https://bioconductor.org/packages/limma/))
...
for (igroup in df_models[["tar_group"]]) {
  ...
  cat(eq(tex = df_models[tar_group %in% igroup, eq], d = TRUE))
  cat("\n")
  print(
